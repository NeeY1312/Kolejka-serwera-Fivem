...
AddEventHandler("playerConnecting", function(name, reject, def)
    ...
    if playerCount >= 80 then
        setReason('NeeYSerwer | Serwer aktualnie jest pełny (80/80)')
        CancelEvent()
    end
...
function Rocade(steamID, licenseID, def, source)
    ...
    if WhitelistStatus then
        ...
        if wl == 2 then
            def.done('Nie znajdujesz się na allow liscie. Aplikuj na discord.gg/discord')
            CancelEvent()
            return
        elseif wl ~= 1 then
            def.done(ticket)
...
function SetIdentity(spawn, source)
    ...
    MySQL.Async.execute("UPDATE `users` SET `position` = '"..position.."' WHERE `identifier` = '"..GetLicenseID(source).."'")
end
